...
def ask_product_question_with_parser(question: str) -> Product:
    ...
    try:
        ...
        if isinstance(response, dict):
            # Extract name
            name = response.get('name', response.get('model', 'Unknown Product'))
            
            # Extract details - combine multiple fields if needed
            details_parts = []
            
            # Add display info if available
            if 'display' in response:
                display = response['display']
                if isinstance(display, dict):
                    display_info = f"Display: {display.get('size', '')} {display.get('type', '')}"
                    details_parts.append(display_info)
                else:
                    details_parts.append(f"Display: {display}")
            
            # Add processor info if available
            if 'processor' in response or 'processors' in response:
                proc = response.get('processor', response.get('processors', {}))
                if isinstance(proc, dict):
                    proc_info = f"Processor: {proc.get('chip', proc.get('type', ''))}"
                    details_parts.append(proc_info)
            
            # Add camera info if available
            if 'camera' in response:
                camera = response['camera']
                if isinstance(camera, dict):
                    details_parts.append("Camera: Advanced camera system")
            
            # Add release date if available
            if 'release_date' in response or 'releaseDate' in response:
                release = response.get('release_date', response.get('releaseDate', ''))
                details_parts.append(f"Released: {release}")
            
            # Combine details or use fallback
            details = response.get('details', '; '.join(details_parts) if details_parts else 'No details available')
            
            # Extract price - handle various formats
            price = 0
            price_field = response.get('price', response.get('startingPrice', 0))
            
            if isinstance(price_field, dict):
                # Handle nested price object
                price_str = price_field.get('startingPrice', price_field.get('price', '0'))
            else:
                price_str = price_field
        ...
        elif isinstance(response, Product):
            return response
        else:
